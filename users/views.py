...
def create(request):
	if request.method == 'POST':
		...
		if User.objects.filter(email=email).exists():
			...
			bankaccout = Useraccount.objects.create(user = instance,account_number=bankaddress,default=True,deposited=float("0.00"))
			bankaccout.save()
			wallet_id_number = random.randint(1000000, 9000000)
			walletaddress = Userwallet.objects.create(user = instance,wallet_id=wallet_id_number,deposited='0.00')
			walletaddress.save()
